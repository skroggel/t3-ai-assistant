 ...
 * Class AdapterItems
 *
 * Provides registered index source adapters as TCA select items.
 *
...
final class AdapterItems
{
    ...
    /**
     * Adds registered adapters to the TCA item list.
     *
    ...
    /**
     * Appends registered adapters to the given TCA parameters.
     *
     * @param array<string, mixed> $parameters TCA parameters.
     * @return void
     */
    protected function appendItems(array &$parameters): void
    {
        ...
        foreach ($this->adapterRegistry->all() as $adapter) {
            $identifier = trim((string)$adapter->getIdentifier());
